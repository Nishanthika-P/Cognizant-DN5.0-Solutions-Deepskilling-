...
def test_window_size():
    ...
    many modern web apps render different layouts (or hide/collapse
    elements, e.g., a hamburger menu vs a full navbar) depending on
    viewport width. If tests run with an inconsistent or default window
    size, a locator that works at one size may not exist (or may be
    hidden/obscured) at another, producing flaky, environment-dependent
    failures. Explicitly setting a fixed size makes test runs
