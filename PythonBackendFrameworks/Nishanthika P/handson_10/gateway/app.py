"""
gateway/app.py
Hands-On 10, Task 2, step 102: a minimal API Gateway that proxies
requests to the correct backend service based on the URL prefix.

  /api/courses/*  -> Course Service  (http://127.0.0.1:5001)
  /api/students/*  -> Student Service (http://127.0.0.1:5002)

A real gateway would also handle auth, rate limiting, and SSL
termination - this demonstrates only the routing concept.
"""
...
def proxy(resource, path):
    ...
    try:
        ...
        return {'error': f'{resource} service is unavailable'}, 503
