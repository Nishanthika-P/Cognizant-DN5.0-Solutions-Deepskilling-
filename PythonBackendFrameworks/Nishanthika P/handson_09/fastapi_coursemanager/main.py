...
(builds on the Hands-On 8 versioned/REST-best-practices FastAPI app).

New in this hands-on:
  - /api/v1/auth/register/ and /api/v1/auth/login/ (auth.py)
  - POST/DELETE on /api/v1/courses/ now require a valid JWT
    (Depends(get_current_user)) - Task 2, step 93
  - CORSMiddleware allowing http://localhost:3000 - Task 2, step 94
"""
...
from models import Course as CourseModel, Student as StudentModel, Enrollment as EnrollmentModel, User as UserModel
from schemas import (
...
app = FastAPI(
    ...
    description='Versioned, REST-best-practices, JWT-secured API - Hands-On 9.',
    version='1.0.0',
)
...
# Task 2, step 94: allow the frontend dev server to call this API.
# CORS is enforced by the BROWSER, not the server - it does not protect
# server-to-server calls (e.g. curl, Postman, another backend).
app.add_middleware(
    CORSMiddleware,
...
async def create_course(
    ...
    current_user: UserModel = Depends(get_current_user),  # Task 2, step 93: auth required
):
